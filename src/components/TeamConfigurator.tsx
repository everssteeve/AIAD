      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem' }}>
        {configs.map((c, i) => (
          <button
            ...
            style={{
              padding: '0.4rem 1rem', borderRadius: '1rem', border: '1px solid',
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {config.members.map((m) => (
            <tr key={m.person}>
              <td style={{ padding: '0.5rem', borderBottom: '1px solid var(--sl-color-gray-6)', fontWeight: 600 }}>{m.person}</td>
              <td style={{ padding: '0.5rem', borderBottom: '1px solid var(--sl-color-gray-6)' }}>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem' }}>
                  {m.responsibilities.map((r) => (
                    <span key={r} style={{ padding: '0.15rem 0.5rem', background: 'var(--sl-color-accent-low)', borderRadius: '0.5rem', fontSize: '0.8rem' }}>{r}</span>
                  ))}
                </div>
              </td>
            </tr>
